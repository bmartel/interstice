...
export type CreateEntityReturn<Value extends { id: string }, InputValue = Value> = (() => [
  { entity?: PartialWithId<Value>; loading: boolean; error?: CombinedError },
  {
    create: (entity: Partial<InputValue> | { [k: string]: Partial<InputValue> }) => Promise<void>
    reset: () => void
  }
]) & {
...
export const createEntity = <Value extends { id: string }, InputValue = Value>(
  ...
  const createSourceAtom = atom(null, (get, set, entity: Partial<Value>) => {
    if (atomEntityInstance.idKey in entity) {
      set(entityIdAtom as any, entity[atomEntityInstance.idKey])
    }
    ...
    if (typeof createSource !== 'boolean' && createSource) {
      const prev = get(createSource as WritableAtom<Value[], Value[]>) as Value[]
      set(createSource as WritableAtom<Value[], Value[]>, [entity as any, ...prev])
    }
  })

...
export const findManyEntities = <Value extends { id: string }>(
  ...
  const entitiesAtom =
    ...
    atom(
      (get) =>
        get(entityIdsAtom)
          ?.map((id) => get(atomEntityInstance({ [atomEntityInstance.idKey]: id } as any)))
          ?.filter(Boolean),
  ...
  function findManyHook(vars?: any) {
    ...
    const [{ fetching, error: _error, data }, refetchQuery] = useQuery({
      query,
      variables: useMemo(() => formatVariables(variables), [variables, formatVariables]),
      pause,
    })
    ...
    useEffect(() => {
      if (!fetching && hasData(data)) {
        if (paginated) {
          ...
          setEntities([...(entitiesRef.current || ([] as any)), ...(items || [])])

...
const mergeEntity = <E = any>(
  ...
  if (relatedEntity?.[lookupAtom.idKey]) {
    ...
    let mergeOriginal = {}
    if (lookupAtom.idKey !== lookupAtom.targetKey && relatedEntity?.[lookupAtom.targetKey]) {
      const original = get(
        lookupAtom({
          [lookupAtom.targetKey]: relatedEntity[lookupAtom.targetKey].toString(),
        })
      )
...
const mergeEffect = <E = any>(relatedEffect: E, set: any, lookupAtom: any): string => {
  const id = relatedEffect[lookupAtom.idKey].toString()
  ...
  return get(
    lookupAtom(
      typeof entityRef === 'object'
        ? entityRef
        : ({
            [lookupAtom.idKey]: entityRef?.[lookupAtom.idKey] || entityRef || defaultId,
          } as any)
    )
  )
}

export const atomEntity = <Value extends { [k: string]: any }, IdType = { id: string }>(
  related?: {
    ...
    (init) => {
      ...
      return atom(
        (get) => {
          ...
          if (related) {
            return Object.entries(related).reduce(
              (resolved, [key, lookup]: [string, AtomEntity | AtomEffect]) => ({
                ...resolved,
                [key]: Array.isArray(lookup)
                  ...
                  : get(lookup({ [(lookup as any).idKey]: lookupId, ...entity } as any)) ||
                    entity?.[key],
              }),
              entity
            )
        ...
        (get, set, update) => {
          ...
          const targetUpdate = update[targetKey] ? { [update[targetKey]]: null } : {}
          const next = {
            ...prev,
            ...targetUpdate,
            [entityId]: {
            ...
            Object.entries(related).map(([key, lookup]) => {
              if (Array.isArray(lookup)) {
                ...
                next[entityId][key] = next[entityId][key]
                  ?.map((relatedEntity: any) => mergeEntity(relatedEntity, get, set, lookupAtom))
                  .filter(Boolean)
              } else {
                const lookupAtom = lookup as any

                // only update if the incoming value is not just the reference to the effect
                if (lookupAtom._name === 'atomEffect' && update[key] !== entityId) {
                  next[entityId][key] =
                    mergeEffect(
                  ...
                  next[entityId][key] = mergeEntity(update[key], get, set, lookupAtom) || entityId
                }
              }
            })
          }

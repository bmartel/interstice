...
`;
const fullWidthInput = litElement.html`
  <style>
...
class FormInput extends Validation(litElement.LitElement) {
  ...
  renderMessages() {
    if (this.messages.length) {
      return litElement.html`
        <ul class="messages">
          ${this.messages.map(msg => litElement.html`<li>${msg}</li></ul>`)}
        </ul>
      `;
    }
...
class CheckBoxGroup extends FormSelect {
  ...
  renderOption(option, index) {
    const optionProps = _objectWithoutProperties(option, ["inline", "reversed", "wide"]);

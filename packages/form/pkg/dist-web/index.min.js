import{LitElement,html,css}from"lit-element";import{classMap}from"lit-html/directives/class-map";import{spread}from"@open-wc/lit-helpers";const Validation=e=>(class extends e{static get properties(){return{rules:Array,messages:Array}}constructor(){super(),this.rules=[]}inputValue(){return this.value}inputName(){return this.name}validate(){this.messages=this.rules.filter(e=>e(this.inputValue())),this.status=this.messages.length?"error":"",this.dispatchEvent("validation",{composed:!0,bubbles:!0,detail:{name:this.inputName(),errors:this.messages}})}});class Form extends LitElement{static get properties(){return{valid:Boolean,value:Object,action:String,method:String,enctype:String}}constructor(){super(),this.value={},this.valid=!1,this.action="",this.method="POST",this.enctype="application/json",this.addEventListener("validation",e=>{this.valid=e.detail.errors.length<1})}updateField(e){try{this.value[e.target.name]=e.target.value,e.target.validate()}catch(e){}}body(){if("application/json"!==this.enctype){const e=new FormData;return Object.keys(this.value).forEach(t=>{e[t]=this.value[t]}),e}return JSON.stringify(this.value)}headers(){return"multipart/form-data"!==this.enctype?{"Content-Type":this.enctype}:{}}async submit(){const{action:e,method:t,valid:r}=this;if(e&&r)try{const r=await fetch(e,{method:t,credentials:"include",body:this.body(),headers:this.headers()}),s=await r.json();this.dispatchEvent(new CustomEvent("form-success",{composed:!0,bubbles:!0,detail:{...s}}))}catch(e){this.dispatchEvent(new CustomEvent("form-error",{composed:!0,bubbles:!0,detail:{error:e}}))}}render(){return html`
      <form @submit=${this.submit} @change=${this.updateField}>
        <slot></slot>
      </form>
    `}}customElements.define("i-form",Form);const fontStyles=html`
  <style>
    :host {
...
`;class FormInput extends(Validation(LitElement)){static get properties(){return{id:{type:String},name:{type:String},type:{type:String},value:{type:String},label:{type:String},hint:{type:String},placeholder:{type:String},messages:{type:Array},wide:{type:Boolean},inline:{type:Boolean},reversed:{type:Boolean},disabled:{type:Boolean},readonly:{type:Boolean},status:{type:String}}}static get styles(){return css`
      :host {
        display: inline-flex;
      }
      .input-wrapper {
    ...
    `}constructor(){super(),this.id="",this.name="",this.value=null,this.label="",this.hint="",this.placeholder="",this.messages=[],this.wide=!1,this.status="",this.inline=!1,this.reversed=!1,this.disabled=!1,this.readonly=!1}get error(){return"error"===this.status}set error(e){}get success(){return"success"===this.status}set success(e){}labelId(){return`label-${this.id}`}hintId(){return`hint-${this.id}`}inputName(){return this.name||this.id}renderLabel(){return this.label?html`
      ...
      <input
        ...
        ...=${spread(e)}
      />
    `}renderMessages(){return this.messages.length?html`
        <ul class="messages">
          ${this.messages.map(e=>html`<li>${e}</li></ul>`)}
        </ul>
      `:null}render(){return html`
      ${fontStyles} ${this.wide?fullWidthInput:null}
      <label
        class=${classMap({"input-wrapper":!0,inline:this.inline,reversed:this.reversed,disabled:this.disabled,error:this.error,success:this.success})}
      >
        ${this.renderHint()} ${this.renderLabel()} ${this.renderInput()} ${this.renderMessages()}
      </label>
    `}}class TextField extends FormInput{constructor(){super(),this.type="text"}}customElements.define("i-textfield",TextField);class TextArea extends FormInput{static get properties(){return{rows:{type:Number}}}static get styles(){return css`
      ${FormInput.styles}
      textarea {
        min-height: calc(var(--font-size) * 7.7);
      }
    `}constructor(){super(),this.rows=4}renderInput(){return html`
      <textarea
        class="input"
        .id=${this.id}
        .value=${this.value}
      ...
      >
    `}}customElements.define("i-textarea",TextArea);const isObject=e=>null!=e&&"object"==typeof e&&"[object Object]"===Object.prototype.toString.call(e);class FormSelect extends FormInput{static get properties(){return{value:{type:Array},dependent:{type:Boolean},dependentOptions:{type:Object},options:{type:Array},inline:{type:Boolean},reversed:{type:Boolean},complex:{type:Boolean},labelKey:{type:String},valueKey:{type:String}}}static get styles(){return css`
      ${FormInput.styles}
      .select-group {
        display: grid;
        grid-gap: var(--z-gap);
    ...
    `}}class Range extends FormInput{static get properties(){return{min:{Type:Number},max:{Type:Number},step:{Type:Number}}}static get styles(){return css`
      ${FormInput.styles}
      .input {
        border: none;
        padding: 0px;
    ...
    `}constructor(){super(),this.type="radio"}renderCheck(){return this.checked?html`
        <svg class="check radio" viewBox="0 0 24 24" preserveAspectRatio="xMidYMid meet">
          <circle cx="12" cy="12" r="6"></circle>
        </svg>
      `:null}}customElements.define("i-radio",Radio);class CheckBoxGroup extends FormSelect{static get styles(){return css`
      ${FormSelect.styles}
      .messages {
        --gap: 0;
      }
    ...
    `}renderOption(e,t){const{inline:r,reversed:s,wide:i,...o}=e;return html`
      <i-checkbox
        .name=${this.name}
        .id=${this.inputId(t)}
        ...
        ...=${spread(o)}
      ></i-checkbox>
    `}}customElements.define("i-checkboxgroup",CheckBoxGroup);class RadioGroup extends FormSelect{static get properties(){return{value:{type:String}}}static get styles(){return css`
      ${FormSelect.styles}
      .messages {
        --gap: 0;
      }
    ...
    `}constructor(){super(),this.value=""}inputUpdate(e){this.value=e.target.value}inputSelected(e){return this.value===this.convertValue(e)}renderOption(e,t){const{inline:r,reversed:s,wide:i,...o}=e;return html`
      <i-radio
        .name=${this.name}
        .id=${this.inputId(t)}
        .label=${this.convertLabel(e)}
        ...
        ...=${spread(o)}
      ></i-radio>
    `}}customElements.define("i-radiogroup",RadioGroup);export{CheckBox,CheckBoxGroup,Form,FormInput,FormSelect,Radio,RadioGroup,Range,TextArea,TextField,Validation};

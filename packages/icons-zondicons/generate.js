...
fs.readdir(iconsSourceDir, (err, files) => {
    ...
    availableIcons.push(
      new Promise(resolve => {
        fs.readFile(iconsSourcePath(file), { encoding: 'utf-8' }, (fileErr, data) => {
          if (!fileErr) {
            let svgData = data.replace(/(<svg([^>]+)>)/gi, '');
  ...
  Promise.all(availableIcons).then(iconList => {
    ...
    fs.writeFile(
      iconsTargetPath('_icons'),
      `export default [${existingIcons.map(i => `'${i}'`).join(',')}];`,
      iconListErr => {
        if (iconListErr) {
    ...
    const iconsIndex = `${existingIcons
      .map(i => `export { default as ${_.startCase(i).replace(/ /g, '')} } from './${i}.js';`)
      .join('\n')}`;
...
import Icon from '../../icon/src/index.js';
customElements.define('i-icon', Icon);

<Meta
  title="Icons - Zondicons"
  parameters={{
...
### Installation
` +
      "\n```bash\n yarn add @interstice/icon\n```\n\n```js\n  import Icon from '@interstice/icon';\ncustomElements.define('i-icon', Icon);\n```" +
      `
## API
